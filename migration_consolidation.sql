...
BEGIN
  ...
  ---------------------------------------------------
  -- 2. EXTEND FIXED BOOKINGS (New Logic)
  ---------------------------------------------------
  -- Calculate the "Source" week (Previous week relative to the target range)
  -- The target range (p_start_date) is Week 1 (Next Week).
  -- We want to look at Week 0 (Current Week) or generally week - 1.
  ...
  LOOP
    ...
    -- Check if target date is within our consolidation range (It should be)
    IF target_date >= p_start_date AND target_date <= p_end_date THEN
       
       -- Check if slot is already taken in the target week
       ...
       IF existing_check = 0 THEN
          ...
          VALUES (
            ...
            true, -- Keep it fixed so it propagates again next week
            now()
          );
       END IF;
    END IF;
